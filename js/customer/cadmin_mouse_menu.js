function load_remarks(){
	...
	$.post( "admin_customer_database.php", {show_remarks: "1"}) 
	...
	$('#contex-of-mousemenu').append("<li><a href='#'>Замечание</a></li>");
...
function contextmenu() {      
	if (document.getElementById('contex-of-mousemenu').children.length <= 1){
			load_remarks();
		}
...
function addremark(text){
	$('.context-menu').remove();
	var rem_td = $('#selected-html-element').parent().children(':nth-child(7)')
	...
	var code = $('#selected-html-element').parent().children(':nth-child(2)').text().trim();
			...
			$.post( "admin_customer_database.php", { change: code, fild: "`remark`", val: "'"+remark+"'"})
			.success(function( data ) {
				
				$.post( "admin_customer_database.php", { change: code, fild: "`remark_off_time`", val: "NULL"})
				.success(function( data ) {
