...
function addremark(text){
	...
	var code = $('#selected-html-element').parent().children(':nth-child(2)').text().trim();
			...
			$.post( "basic_customer_database.php", { change: code, fild: "`remark`", val: "'"+remark+"'"})
			.success(function( data ) {
				
				$.post( "basic_customer_database.php", { change: code, fild: "`remark_off_time`", val: "NULL"})
				.success(function( data ) {
	
					$.post( "basic_customer_database.php", { show: 'code', val: code})
					.success(function( data ) {
